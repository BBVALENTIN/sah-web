...
function addLobbyRow(lobby: lobbyInfo) {
    ...
    if (createdRow) {
        createdRow.innerHTML = `
           <td data-label="Creator">${creator}</td>
            <td data-label="Lobby Id">${lobby.lobbyId}</td>
            <td data-label="Status">Open</td>
            <td data-label="Action"><button class="join-btn" data-id="${lobby.lobbyId}">Join</button></td>
        `;
    } else {
